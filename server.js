...
// ====== CONFIG.JS INJECTION (WORLD CLASS) ======
app.get('/config.js', (req,res)=>{
  const url=process.env.SUPABASE_URL, key=process.env.SUPABASE_KEY;
  if(!url||!key) return res.type('application/javascript').send(`console.error("Missing SUPABASE env - check Render variables");`);
...
// 🔥 WORLD CLASS STATIC SERVING - FIX YA NOT FOUND
const publicPath = path.join(__dirname,"public");
app.use(express.static(publicPath, { maxAge: '1d', etag: true }));
app.use(express.static(__dirname, { maxAge: '1d' })); // fallback if files in root
...
// ====== AUTH ROUTES (YOUR CODE MAINTAINED) ======
...
// ====== GUEST ORDERS (WORLD CLASS SECURED WITH SERVICE KEY) ======
...
// ====== WORLD CLASS FRONTEND ROUTES - FIX YA NOT FOUND 100% ======

// Health
app.get("/api/health", (req,res)=> res.json({ok:true, time:new Date(), service:"GuestHub OS 11.0 Professional"}));

// Explicit dashboard routes (professional)
const sendPublic = (file) => (req,res) => {
  const full = path.join(publicPath, file);
  res.sendFile(full, (err)=>{
    if(err){
      // fallback to root if public/ missing
...
app.get("/hotel-dashboard", sendPublic("hotel-dashboard.html"));
app.get("/hotel-dashboard.html", sendPublic("hotel-dashboard.html"));
app.get("/vendor", sendPublic("vendor-dashboard.html"));
app.get("/vendor-dashboard", sendPublic("vendor-dashboard.html"));
app.get("/vendor-dashboard.html", sendPublic("vendor-dashboard.html"));

// Root = Homepage (index.html)
app.get("/", sendPublic("index.html"));

// Catch all - if api 404, else serve index for SPA
app.get("*", (req,res)=>{
